...
**Alert endpoints (Steps 1–2) in cloud sessions:** the Claude cloud sandbox proxies ALL api.github.com traffic and rewrites the Authorization header, so the REST alert endpoints (`dependabot/alerts`, `secret-scanning/alerts`, `code-scanning/alerts`) return 403 no matter which token is used — a dedicated PAT in an env var does not help (verified 2026-09-07 against both org- and repo-level paths). In cloud sessions, pull alert data through an attached read-only GitHub MCP connector (GitHub's hosted MCP server, e.g. named `github-security`) instead: discover and use its Dependabot / secret-scanning / code-scanning alert list/get tools. If those tools are repo-scoped rather than org-scoped, enumerate the org's non-archived repos first and iterate, then aggregate to the org-wide views below. If no such connector is attached or its tools all error, skip Steps 1–2, say exactly why in the digest, and continue with Step 3.
